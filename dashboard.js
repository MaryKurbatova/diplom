    ...
    try {
        ...
        if (d.boards && d.boards.length) {
            for (var i = 0; i < d.boards.length; i++) {
                var b = d.boards[i];
                boardsHtml += '<div class="detail-row"><span class="detail-label">' + escapeHtml(b.board_type_name || 'Плата') + ' (' + escapeHtml(b.serial_number) + ')</span><span class="detail-value"><span class="badge ' + stageBadge(b.current_stage) + '">' + stageLabel(b.current_stage) + '</span></span></div>';
            }
        } else {
...
async function loadBoards() {
    ...
    try {
        ...
        var rsBoards = 0, saBoards = 0;
        for (var i = 0; i < S.boards.length; i++) {
            var sn = S.boards[i].serial_number || '';
            if (sn.includes('-RS-') || sn.startsWith('PB-RS-')) {
                rsBoards++;
            } else if (sn.includes('-SA-') || sn.startsWith('PB-SA-')) {
                saBoards++;
            }
        }
        ...
        html += '<div class="stat-card"><div class="stat-info"><span class="stat-value">' + rsBoards + '</span><span class="stat-label">Для RS</span></div></div>';
        html += '<div class="stat-card"><div class="stat-info"><span class="stat-value">' + saBoards + '</span><span class="stat-label">Для SA</span></div></div>';
...
function applyBoardFilters() {
    ...
    S.filteredBoards = S.boards.filter(function(b) {
        ...
        const isRS = (b.serial_number || '').includes('-RS-') || (b.serial_number || '').startsWith('PB-RS-');
        const isSA = (b.serial_number || '').includes('-SA-') || (b.serial_number || '').startsWith('PB-SA-');

        if (typeFilter === 'router') return isRS;
        if (typeFilter === 'switch') return isSA;
        return true;
    });
...
function renderBoardRows() {
    ...
    for (var i = 0; i < S.filteredBoards.length; i++) {
        ...
        let deviceTypeLabel = '—';
        let deviceTypeClass = 'badge-neutral';
        if (b.serial_number.includes('-RS-') || b.serial_number.startsWith('PB-RS-')) {
            deviceTypeLabel = 'RS';
            deviceTypeClass = 'badge-info';
        } else if (b.serial_number.includes('-SA-') || b.serial_number.startsWith('PB-SA-')) {
            deviceTypeLabel = 'SA';
            deviceTypeClass = 'badge-success';
        }
        
        html += '<tr>';
        html += '<td><strong style="color:var(--primary);cursor:pointer" onclick="showBoardDetails(' + b.id + ')">' + escapeHtml(b.serial_number) + '</strong></td>';
        html += '<td><span class="badge ' + deviceTypeClass + '">' + deviceTypeLabel + '</span></td>';
        ...
        if (canEditFlag) {
            ...
            html += '<button class="btn-icon" onclick="showBoardDetails(' + b.id + ')" title="Управление статусом"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 14.66V20a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h5.34"></path><polygon points="18 2 22 6 12 16 8 16 8 12 18 2"></polygon></svg></button>';
            html += '<button class="btn-icon danger" onclick="deleteBoard(' + b.id + ')" title="Удалить"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button>';
            html += '</div></td>';
        }
        html += '</tr>';
    }
...
function showAddBoard() {
    openModal('Новая плата',
        ...
        '<div class="form-grid">' +
        '<div class="form-group"><label class="form-label">Серийный номер *</label><input class="form-input" name="serial_number" required placeholder="PB-RS-2025-001 или PB-SA-2025-001"></div>' +
        '</div>' +
...
async function saveBoard(event) {
    ...
    for (var i = 0; i < S.boardTypes.length; i++) {
        if (S.boardTypes[i].code === 'BOARD') {
            boardTypeId = S.boardTypes[i].id;
            break;
        }
    }
    
    if (!boardTypeId && S.boardTypes.length > 0) {
        boardTypeId = S.boardTypes[0].id;
    }
...
// ============ УПРАВЛЕНИЕ СТАТУСАМИ ПЛАТ ============

async function updateBoardVisualStatus(boardId, passed, comment) {
    try {
        const board = await api('/api/boards/' + boardId);
        if (!board) throw new Error('Плата не найдена');
        
        if (board.current_stage !== 'new' && board.current_stage !== 'visual_fail') {
            showError(`Нельзя изменить статус. Текущая стадия: ${board.current_stage}`);
            return false;
        }
        
        const now = new Date().toISOString().slice(0, 19).replace('T', ' ');
        const result = passed ? 1 : 0;
        const newStage = passed ? 'visual_ok' : 'visual_fail';
        
        const updateData = {
            visual_inspection_passed: passed,
            visual_inspection_date: now,
            visual_inspection_employee_id: S.user.id,
            visual_inspection_comment: comment || (passed ? 'Осмотр пройден' : 'Брак'),
        ...
        await api('/api/visual-inspection-records', {
            method: 'POST',
            body: JSON.stringify({
                board_id: boardId,
                employee_id: S.user.id,
                inspection_date: now,
                result: passed,
                comment: comment || (passed ? 'Осмотр пройден' : 'Брак пайки')
            })
        });
        ...
        if (passed) {
            ...
            toast('Плата отправлена в ремонт ✗', 'error');
            showStandResult('Плата отправлена в ремонт', 'error');
        }
...
async function updateBoardDiagnosticsStatus(boardId, passed, comment, details = {}) {
    try {
        const board = await api('/api/boards/' + boardId);
        if (!board) throw new Error('Плата не найдена');
        
        if (!board.visual_inspection_passed && board.current_stage !== 'diagnostics_fail') {
            showError('Нельзя пройти диагностику без визуального осмотра');
            return false;
        }
        
        if (board.current_stage !== 'visual_ok' && board.current_stage !== 'diagnostics_fail') {
            showError(`Нельзя изменить статус. Текущая стадия: ${board.current_stage}`);
            return false;
        }
        ...
        const now = new Date().toISOString().slice(0, 19).replace('T', ' ');
        const result = passed ? 1 : 0;
        const newStage = passed ? 'diagnostics_ok' : 'diagnostics_fail';
        
        const updateData = {
            diagnostics_passed: passed,
            diagnostics_date: now,
            diagnostics_employee_id: S.user.id,
            diagnostics_comment: comment || (passed ? 'Диагностика пройдена' : 'Ошибка теста'),
            diagnostics_ip: details.ip_address || null,
            diagnostics_stand: details.stand_name || null,
            current_stage: newStage
        };
        ...
        await api('/api/diagnostics-records', {
            method: 'POST',
            body: JSON.stringify({
                board_id: boardId,
                employee_id: S.user.id,
                ...
                stand_name: details.stand_name || null,
                ports_ok: details.ports_ok || false,
                os_installed: details.os_installed || false,
                disks_ok: details.disks_ok || false,
                memory_ok: details.memory_ok || false
            })
        });
...
async function resetBoardToNew(boardId) {
    if (!confirm('Сбросить плату в статус "Новая"? Все данные о прохождении будут удалены.')) return;
    
    try {
        const updateData = {
            current_stage: 'new',
            visual_inspection_passed: false,
            visual_inspection_date: null,
            visual_inspection_employee_id: null,
            visual_inspection_comment: null,
            diagnostics_passed: false,
            diagnostics_date: null,
            diagnostics_employee_id: null,
            diagnostics_comment: null,
            diagnostics_ip: null,
            diagnostics_stand: null,
            assembly_passed: false,
            assembly_date: null,
            assembly_employee_id: null,
            device_id: null
        };
        ...
        toast('Плата сброшена в статус "Новая"', 'success');
        await refreshCurrentSectionData();
        closeModal();
...
function showDiagnosticsModal(boardId) {
    openModal('Диагностика платы', `
        <form onsubmit="submitDiagnosticsFromModal(event, ${boardId})">
            <div class="form-grid">
                <div class="form-group full-width">
                    <label class="form-label">Результат *</label>
                    <select class="form-select" name="result" required>
                        <option value="ok">Пройдена</option>
                        <option value="fail">Не пройдена</option>
                    </select>
                </div>
                <div class="form-group full-width">
                    <label class="form-label">Комментарий</label>
                    <textarea class="form-textarea" name="comment" rows="2" placeholder="Подробности диагностики..."></textarea>
                </div>
                <div class="form-group">
                    <label class="form-label">IP-адрес</label>
                    <input class="form-input" name="ip_address" placeholder="192.168.1.100">
                </div>
                <div class="form-group">
                    <label class="form-label">Стенд</label>
                    <input class="form-input" name="stand_name" placeholder="Стенд Д-1">
                </div>
                <div class="form-group">
                    <label class="form-label"><input type="checkbox" name="ports_ok" value="true"> Порты OK</label>
                </div>
                <div class="form-group">
                    <label class="form-label"><input type="checkbox" name="os_installed" value="true"> ОС установлена</label>
                </div>
                <div class="form-group">
                    <label class="form-label"><input type="checkbox" name="disks_ok" value="true"> Диски OK</label>
                </div>
                <div class="form-group">
                    <label class="form-label"><input type="checkbox" name="memory_ok" value="true"> Память OK</label>
                </div>
            </div>
...
async function submitDiagnosticsFromModal(event, boardId) {
    event.preventDefault();
    const fd = new FormData(event.target);
    const result = fd.get('result') === 'ok';
    const comment = fd.get('comment') || '';
    
    const details = {
        ip_address: fd.get('ip_address') || null,
        stand_name: fd.get('stand_name') || null,
        ports_ok: fd.get('ports_ok') === 'true',
        os_installed: fd.get('os_installed') === 'true',
        disks_ok: fd.get('disks_ok') === 'true',
        memory_ok: fd.get('memory_ok') === 'true'
    };
    
    await updateBoardDiagnosticsStatus(boardId, result, comment, details);
}
...
async function showBoardDetails(id) {
    try {
        ...
        if (b.vi_records && b.vi_records.length) {
            ...
            viHtml = '<p style="color:var(--text-muted)">Нет записей</p>';
        }
        ...
        if (b.diag_records && b.diag_records.length) {
            for (var i = 0; i < b.diag_records.length; i++) {
                ...
                diagHtml += '<div class="detail-row"><span class="detail-label">' + (r.diagnostics_date || '') + '</span><span class="detail-value">' + (r.result ? 'Пройдена ✓' : 'Не пройдена ✗') + ' ' + escapeHtml(r.comment || '') + '</span></div>';
            }
        } else {
            diagHtml = '<p style="color:var(--text-muted)">Нет записей</p>';
        }

        let boardTypeForDevice = '—';
        if (b.serial_number.includes('-RS-') || b.serial_number.startsWith('PB-RS-')) {
            boardTypeForDevice = 'Для маршрутизатора (RS)';
        } else if (b.serial_number.includes('-SA-') || b.serial_number.startsWith('PB-SA-')) {
            boardTypeForDevice = 'Для коммутатора (SA)';
        }
        ...
        html += '<div class="detail-row"><span class="detail-label">Тип платы</span><span class="detail-value">' + escapeHtml(b.board_type_name || '—') + '</span></div>';
        html += '<div class="detail-row"><span class="detail-label">Для устройства</span><span class="detail-value"><span class="badge ' + (boardTypeForDevice.includes('RS') ? 'badge-info' : 'badge-success') + '">' + boardTypeForDevice + '</span></span></div>';
        ...
        if (S.user && S.user.role !== 'operator') {
            html += '<div class="section-card" style="margin-top:20px">';
            html += '<h3>Управление статусом</h3>';
            html += '<div class="form-grid">';
            
            if (b.current_stage === 'new' || b.current_stage === 'visual_fail') {
                html += '<div class="form-group">';
                html += '<label class="form-label">Визуальный осмотр</label>';
                html += '<div style="display:flex; gap:8px; margin-top:8px; flex-wrap:wrap;">';
                html += '<button class="btn btn-sm" onclick="updateBoardVisualStatus(' + b.id + ', true, \'Осмотр пройден\')" style="background:#2e7d32;color:white;padding:8px 16px;border:none;border-radius:8px;cursor:pointer">✓ Пройден</button>';
                html += '<button class="btn btn-sm" onclick="updateBoardVisualStatus(' + b.id + ', false, \'Брак пайки\')" style="background:var(--primary);color:white;padding:8px 16px;border:none;border-radius:8px;cursor:pointer">✗ Брак</button>';
                html += '</div></div>';
            } else if (b.visual_inspection_passed) {
                html += '<div class="form-group">';
                html += '<label class="form-label">Визуальный осмотр</label>';
                html += '<div style="display:flex; gap:8px; margin-top:8px;">';
                html += '<span class="badge badge-success" style="padding:8px 16px;">✓ Пройден</span>';
                html += '</div></div>';
            }
            
            if (b.visual_inspection_passed || b.current_stage === 'diagnostics_fail') {
                html += '<div class="form-group">';
                html += '<label class="form-label">Диагностика</label>';
                html += '<div style="display:flex; gap:8px; margin-top:8px;">';
                html += '<button class="btn btn-sm" onclick="showDiagnosticsModal(' + b.id + ')" style="background:var(--primary);color:white;padding:8px 16px;border:none;border-radius:8px;cursor:pointer">▶ Пройти диагностику</button>';
                html += '</div></div>';
            } else if (b.diagnostics_passed) {
                html += '<div class="form-group">';
                html += '<label class="form-label">Диагностика</label>';
                html += '<div style="display:flex; gap:8px; margin-top:8px;">';
                html += '<span class="badge badge-success" style="padding:8px 16px;">✓ Пройдена</span>';
                html += '</div></div>';
            }
            
            if (b.current_stage === 'visual_fail' || b.current_stage === 'diagnostics_fail') {
                html += '<div class="form-group">';
                html += '<label class="form-label">Сброс</label>';
                html += '<div style="display:flex; gap:8px; margin-top:8px;">';
                html += '<button class="btn btn-sm" onclick="resetBoardToNew(' + b.id + ')" style="background:var(--bg-input);color:var(--text-primary);padding:8px 16px;border:1px solid var(--border);border-radius:8px;cursor:pointer">⟳ Сбросить в новую</button>';
                html += '</div></div>';
            }
...
async function refreshCurrentSectionData() {
    ...
    try {
        ...
        if (devices) S.devices = devices;
        
        const boards = await api('/api/boards');
        if (boards) {
            S.boards = boards;
            S.filteredBoards = boards.slice();
        }
        
        if (devices) {
            S.repairDevices = devices.filter(d => 
                ['visual_fail', 'diagnostics_fail', 'psi_fail'].includes(d.current_stage)
            );
        }
        if (boards) {
            S.repairBoards = boards.filter(b => 
                ['visual_fail', 'diagnostics_fail'].includes(b.current_stage)
            );
        }
...
function renderStandVisual() {
    ...
    contentArea.innerHTML = `
        <div class="stand-workstation">
            <div class="stand-header">
                ...
                <p class="stand-instruction">Отсканируйте или выберите плату. Доступны новые и бракованные (из ремонта).</p>
            </div>
    ...
    if (input) {
        input.addEventListener('focus', async function() {
            try {
                const boards = await api('/api/boards');
                const availableBoards = boards.filter(b => b.current_stage === 'new' || b.current_stage === 'visual_fail');
...
function renderStandDiag() {
    ...
    contentArea.innerHTML = `
        <div class="stand-workstation">
            <div class="stand-header">
                ...
                <p class="stand-instruction">Выберите плату, прошедшую визуальный осмотр, или из ремонта.</p>
            </div>
            <div class="stand-form">
                <div class="stand-input-group">
                    <label>Серийный номер платы</label>
    ...
    if (input) {
        input.addEventListener('focus', async function() {
            try {
                const boards = await api('/api/boards');
                const availableBoards = boards.filter(b => b.current_stage === 'visual_ok' || b.current_stage === 'diagnostics_fail');
        ...
        <div class="stand-workstation">
            <div class="stand-header">
                ...
                <p class="stand-instruction">
                    Сканируйте плату (прошедшую диагностику). Серийный номер изделия сгенерируется автоматически.
            ...
            <div class="stand-form">
                <div class="stand-input-group">
                    <label>Серийный номер ПЛАТЫ <span style="color: var(--text-muted); font-size: 12px;">(Enter для добавления)</span></label>
                    <input type="text" id="asmBoard" class="stand-qr-input" 
                           placeholder="Сканируйте плату..." autocomplete="off">
                </div>
...
async function addBoardToAssembly(serialNumber) {
    ...
    if (scannedBoardsForAssembly.length > 0) {
        showStandResult('Можно добавить только одну плату', 'error');
        return;
    }
    ...
    try {
        ...
        if (board.serial_number.includes('-RS-') || board.serial_number.startsWith('PB-RS-')) {
            deviceType = 'RS';
            deviceTypeId = 1;
        } else if (board.serial_number.includes('-SA-') || board.serial_number.startsWith('PB-SA-')) {
            deviceType = 'SA';
            deviceTypeId = 2;
        } else {
        ...
        scannedBoardsForAssembly.push({
            ...
            name: board.board_type_name || 'Плата',
        ...
        updateAssemblyUI();
        showStandResult('Добавлена плата (' + board.serial_number + ') - тип ' + deviceType, 'success');
                ...
                ${scannedBoardsForAssembly.map(b => `
                    <div style="padding: 8px; margin: 5px 0; background: var(--bg-input); border-radius: 6px; display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            ...
                            <span style="color: var(--text-muted); font-family: monospace; margin-left: 10px;">${b.device_type}</span>
                        </div>
...
async function submitAssembly() {
    ...
    if (scannedBoardsForAssembly.length === 0) {
        showStandResult('Отсканируйте плату', 'error');
        return;
    }
...
async function submitPSI(result) {
    ...
    try {
        ...
        if (result === 'ok') {
            ...
            showStandResult('Устройство отправлено в ремонт', 'error');
            toast('Устройство отправлено в ремонт', 'error');
        }
...
async function submitPackaging() {
    ...
    if (!sn) return showStandResult('Введите серийный номер', 'error');
    showStandResult('Упаковка и печать наклеек...', 'info');
    try {
        ...
        showStandResult('Упаковка завершена! Наклейки отправлены на печать.', 'success');
        
        if (r.sticker_url) {
            toast('Наклейка на коробку сформирована', 'success');
        }
        
        if (r.passport_sticker_url) {
            toast('Наклейка для паспорта сформирована', 'success');
        }

        document.getElementById('packSn').value = '';
        document.getElementById('packSn').focus();
        await refreshCurrentSectionData();
        toast('Упаковка завершена, наклейки отправлены на печать', 'success');
    } catch (e) { 
        showStandResult(e.message, 'error'); 
    }
}
    ...
    try {
        ...
        if (S.repairBoards.length > 0) {
            ...
            for (var i = 0; i < S.repairBoards.length; i++) {
                ...
                html += '<span class="repair-sn">' + escapeHtml(b.serial_number || '—') + deviceTypeLabel + '</span>';
...
async function resumeRepairBoard(boardId, currentStage) {
    try {
        ...
        const board = await api('/api/boards/' + boardId);
        if (!board) {
            showError('Плата не найдена');
            return;
        }

        toast('Переход к стенду ' + (targetStand === 'stand-visual' ? 'визуального осмотра' : 'диагностики'), 'info');
        showContent(targetStand);

        setTimeout(function() {
            var formInput = null;
            if (targetStand === 'stand-visual') {
                formInput = document.getElementById('visualSn');
            } else if (targetStand === 'stand-diag') {
                formInput = document.getElementById('diagSn');
            }
            
            if (formInput) {
                formInput.value = board.serial_number;
                formInput.focus();
                const resultDiv = document.getElementById('standResult');
                if (resultDiv) {
                    resultDiv.className = 'stand-result info';
                    resultDiv.innerHTML = '<h4>Плата из ремонта: ' + board.serial_number + '.</h4><p>Повторите проверку.</p>';
                }
            }
        }, 500);
    } catch (e) {
