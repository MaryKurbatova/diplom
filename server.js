...
// Конфигурация устройств (принимаем любые платы)
const DEVICE_CONFIG = {
    'RS': { name: 'Маршрутизатор (RS)', type: 'ISN41508T3', os: 'RouterOS 6.4' },
    'SA': { name: 'Коммутатор (SA)', type: 'ISN41508T4', os: 'SwitchOS 4.0' }
};
...
function printPDF(filePath) {
    return new Promise((resolve, reject) => {
        exec(`start "" "${filePath}"`, (error) => {
            if (error) {
                console.warn('Не удалось открыть PDF:', error.message);
                reject(error);
            } else {
                resolve(true);
            }
    ...
    try {
        ...
        for (let dev of rows) {
            ...
            const [boards] = await db.query(
                'SELECT b.*, bt.name as board_type_name FROM boards b LEFT JOIN board_type bt ON b.board_type_id = bt.id WHERE b.device_id = ?',
                [dev.id]
            );
            dev.boards = boards;
        }
...
app.get('/api/boards', auth, async (req, res) => {
    try {
        if (req.user.role === 'operator') {
            const [rows] = await db.query(`
                SELECT b.id, b.serial_number, b.current_stage, bt.name as board_type_name
                FROM boards b
                LEFT JOIN board_type bt ON b.board_type_id = bt.id
                ORDER BY b.id DESC
            `);
...
// PUT для обновления платы (нужен для изменения статуса)
app.put('/api/boards/:id', auth, canEdit, async (req, res) => {
    try {
        ...
        const allowedFields = [
            'current_stage',
            'visual_inspection_passed', 'visual_inspection_date', 
            'visual_inspection_employee_id', 'visual_inspection_comment',
            'diagnostics_passed', 'diagnostics_date', 'diagnostics_employee_id',
            'diagnostics_comment', 'diagnostics_ip', 'diagnostics_stand',
        ...
        res.json({ message: 'Плата обновлена' });
    } catch (e) {
...
// POST для visual_inspection_records (создание новой записи)
app.post('/api/visual-inspection-records', auth, canEdit, async (req, res) => {
    try {
        const { board_id, employee_id, inspection_date, result, comment } = req.body;
        const [resultQuery] = await db.query(
            `INSERT INTO visual_inspection_records (board_id, employee_id, inspection_date, result, comment)
             VALUES (?, ?, ?, ?, ?)`,
            [board_id, employee_id, inspection_date, result ? 1 : 0, comment]
        );
        res.json({ id: resultQuery.insertId, message: 'Запись добавлена' });
    } catch (e) {
        console.error('POST /api/visual-inspection-records error:', e);
        res.status(500).json({ error: e.message });
    }
});

// POST для diagnostics_records
app.post('/api/diagnostics-records', auth, canEdit, async (req, res) => {
    try {
        const { board_id, employee_id, diagnostics_date, result, comment, ip_address, stand_name, ports_ok, os_installed, disks_ok, memory_ok } = req.body;
        const [resultQuery] = await db.query(
            `INSERT INTO diagnostics_records (board_id, employee_id, diagnostics_date, result, comment, ip_address, stand_name, ports_ok, os_installed, disks_ok, memory_ok)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [board_id, employee_id, diagnostics_date, result ? 1 : 0, comment, ip_address, stand_name,
             ports_ok ? 1 : 0, os_installed ? 1 : 0, disks_ok ? 1 : 0, memory_ok ? 1 : 0]
        );
        res.json({ id: resultQuery.insertId, message: 'Запись добавлена' });
    } catch (e) {
        console.error('POST /api/diagnostics-records error:', e);
        res.status(500).json({ error: e.message });
    }
});
...
app.post('/api/stands/visual-inspection', auth, async (req, res) => {
    try {
        ...
        if (!boards.length) {
            const [boardTypes] = await db.query('SELECT id FROM board_type WHERE code = ?', ['BOARD']);
        ...
        // Разрешаем повторный осмотр для плат в статусе visual_fail (ремонт)
        if (board.current_stage !== 'new' && board.current_stage !== 'visual_fail') {
            return res.status(400).json({ error: 'Плата уже на стадии: ' + board.current_stage + '. Доступны только новые или бракованные.' });
        }
...
app.post('/api/stands/diagnostics', auth, async (req, res) => {
    try {
        ...
        // Разрешаем диагностику для плат, прошедших осмотр, а также для плат в ремонте (diagnostics_fail)
        ...
        // Разрешаем повторную диагностику для плат со статусом diagnostics_fail
        if (board.current_stage !== 'visual_ok' && board.current_stage !== 'diagnostics_fail') {
            return res.status(400).json({ error: 'Стадия: ' + board.current_stage + '. Нужна visual_ok или diagnostics_fail' });
        }
...
// ОБНОВЛЕННЫЙ СТЕНД СБОРКИ - принимает любые платы
app.post('/api/stands/assembly', auth, async (req, res) => {
    try {
        ...
        // Проверяем стадию плат - принимаем любые платы, прошедшие диагностику
...
app.post('/api/stands/packaging', auth, async (req, res) => {
    try {
        ...
        try {
            ...
            stickerPath = await generateSimpleStickerPDF(device_serial_number, deviceType, article);
            passportPath = await generatePassportStickerPDF(device_serial_number, deviceType);
            
            // Автоматическая печать обоих PDF
            console.log('Печать наклейки на коробку...');
            printPDF(stickerPath).catch(e => console.warn('Не удалось напечатать наклейку:', e.message));
            
            console.log('Печать наклейки для паспорта...');
            printPDF(passportPath).catch(e => console.warn('Не удалось напечатать паспортную наклейку:', e.message));
            
        } catch (e) {
            console.warn('Ошибка при генерации/печати наклеек:', e.message);
        }

        res.json({
            message: 'Упаковка завершена! Наклейки отправлены на печать',
            sticker_url: stickerPath ? '/stikers/' + path.basename(stickerPath) : null,
            passport_sticker_url: passportPath ? '/stikers/' + path.basename(passportPath) : null,
            device_id: device.id,
